...
impl From<sqlx::Error> for ApiError {
	fn from(err: sqlx::Error) -> Self {
		error!("{}:{}:{}", std::file!(), std::line!(), err.to_string());
		error!("{}", Backtrace::force_capture());
		ApiError {
			error: ApiErrorCode::DatabaseError.to_string(),
			error_code: ApiErrorCode::DatabaseError,
			message: Some("Read the logs.".to_string()),
			http_status_code: 450,
		}
	}
}
